...
import { Upload, FileVideo, FileText, ArrowRight, Download, ExternalLink, Music, Wand2, Mic, Play, FileAudio, Disc, Video, Clapperboard, Sparkles, CheckSquare, Edit2, Save, X, Headphones, Trash2 } from 'lucide-react';
...
export const FileUpload: React.FC<FileUploadProps> = ({ onFilesSelected, apiKey }) => {
  ...
  const handleAutoGenerateSRT = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      console.error(err);
      alert("Auto-generation failed. Ensure the file is less than 20MB or use Transcri.io.");
    } finally {
  ...
  return (
    <div className="flex flex-col items-center h-full p-6 max-w-5xl mx-auto space-y-8 animate-fade-in overflow-y-auto">
      
      {/* Header */}
      <div className="text-center space-y-2">
        <h1 className="text-4xl md:text-5xl font-black bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-pink-600 tracking-tight">
          Reel Composer
        </h1>
        <p className="text-gray-400 text-sm">Create viral shorts from any media source.</p>
      </div>

      {/* Simple Tabs - Minimalist Design */}
                     ...
                     <div className="relative w-full h-full flex flex-col items-center justify-center z-10">
                        ...
                        <div className="text-center px-4">
                         ...
                         <button onClick={handleExtractAudio} disabled={isExtracting} className="absolute bottom-4 right-4 z-20 flex items-center gap-1.5 bg-black/60 hover:bg-black/80 backdrop-blur text-xs text-gray-300 px-3 py-1.5 rounded-lg border border-gray-700 transition-colors">
                            {isExtracting ? <span className="animate-pulse">Processing...</span> : <><Download size={12}/> Get Audio</>}
                         </button>
                     </div>
                 ) : (
      ...
      <button
        ...
        className={`group relative overflow-hidden flex items-center justify-center space-x-3 px-12 py-5 rounded-full font-black text-lg transition-all transform w-full max-w-md ${
